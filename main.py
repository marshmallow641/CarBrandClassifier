...
from helper_script import resize_training_images, generate_annotations_csv, dataset_split
...
if __name__ == '__main__':
    ...
    #dataset_split(train_data_path, train_size=0.7)
...
############################################ Training the model ################################################################################################
    ...
    for epoch in range(EPOCHS):
        ...
        for i in range(train_data.__len__() // BATCH_SIZE):
            ...
            training_images = torch.tensor(data=((training_images - training_images_mean) / training_images_std),dtype=torch.float, device=DEVICE)
...
##################################### Testing the saved models ###################################################################################################
    ...
    with torch.no_grad():
        for model_ in models:
            ...
            print(f"Model ({model_}) Accuracy ({test_data.__len__()} instances): {correct} out of {total} predictions were correct (within a {test_margin * 100}% margin) - {int((correct / total) * 100)}% accuracy")
